...
posfilename = 'Vacuole_nodes'
posfilename = 'Dispersed_grazing_nodes'
posfilename = 'Line_nodes'

posDF = pd.read_csv(posfilename+'.csv') 


# convert from GPS coordinates to cartesian coordinates
startLon = posDF['XCOORD'][0]
startLat = posDF['YCOORD'][0]

posDF['xm']=0
posDF['ym']=0
for i,pos in posDF.iterrows():
    diff = Geodesic.WGS84.Inverse(startLat,startLon,pos['YCOORD'],pos['XCOORD'])
    distance = diff['s12']
    angle = math.radians(90-diff['azi1']) # lat lon goes clockwise from north
    posDF['xm'].iloc[i]= distance*math.cos(angle)
    posDF['ym'].iloc[i]= distance*math.sin(angle)
